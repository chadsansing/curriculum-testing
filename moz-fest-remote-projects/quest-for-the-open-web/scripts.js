...
var classes = ['coder', 'teacher', 'captain', 'leader', 'programmer', 'artist', 'painter', 'dreamer', 'writer', 'explorer', 'gamer', 'organizer', 'activist', 'liberator', 'designer', 'maker', 'builder', 'guide', 'mentor', 'educator', 'librarian', 'crafter', 'tinkerer', 'sculptor', 'composer', 'dancer', 'elder', 'counsellor', 'puppeteer', 'director', 'associate', 'lead', 'visionary'];

var origins = ['the Open Web', 'open educational resources', 'youth programming', 'museum education', 'Web-native apps', 'remix', 'mash-up', 'hackery', 'hack jams', 'game development', 'coding for the Web', 'the stack', 'snacks', 'coffee', 'scripting', 'Web styles', 'HTML', 'CSS', 'Web literacy', 'project management', 'debugging', 'localization', 'equity', 'equality', 'diversity', 'inclusion', 'collaboration', 'community', 'contribution ladders', 'the Internet', 'traceroute', 'privacy', 'security', 'encryption', 'inception', 'MozFest', 'Thimble', 'Popcorn', 'Appmaker', 'Firefox OS', 'mobile development', 'open science', 'open journalism', 'meta-wrangling', 'space-wrangling', 'Ravensbourne', 'London', 'open source', 'transparency', 'make it, eff it, ship it', 'dev sprints', 'caffeine', 'storytelling', 'user experience', 'graphic design', 'Turtle Wushu', 'karaoke', 'the commons', 'open data', 'the distro', 'the fork', 'pull requests', 'Open'];
...
this.adjective = adjectives[Math.round(Math.random()*adjectives.length)];
this.class = classes[Math.round(Math.random()*classes.length)];
this.origin = origins[Math.round(Math.random()*origins.length)];
this.buff = buffs[Math.round(Math.random()*3)];
this.skill = skills[Math.round(Math.random()*skills.length)];
